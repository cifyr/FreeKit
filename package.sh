#!/bin/bash
# Package FreeSpeech for sharing. Run ./build.sh first.
...
if [[ "$APP_ONLY" -eq 1 ]]; then
    STAGE="$ROOT/dist/FreeSpeech-share-lite"; ZIP="$ROOT/dist/FreeSpeech-share-lite.zip"
else
    STAGE="$ROOT/dist/FreeSpeech-share"; ZIP="$ROOT/dist/FreeSpeech-share.zip"
...
#!/bin/bash
# FreeSpeech installer: copies the app to /Applications, clears the download quarantine,
# installs a bundled model if present, and launches. Requires Apple Silicon + macOS 26.
set -euo pipefail
DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

echo "Installing FreeSpeech..."
DEST="/Applications/FreeSpeech.app"
[[ -w /Applications ]] || DEST="$HOME/Applications/FreeSpeech.app"
mkdir -p "$(dirname "$DEST")"
rm -rf "$DEST"
cp -R "$DIR/FreeSpeech.app" "$DEST"
...
if compgen -G "$DIR/models/*.bin" > /dev/null; then
    ...
    echo "No bundled model — FreeSpeech downloads it on first launch (needs internet once)."
fi
